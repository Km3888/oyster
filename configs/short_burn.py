...
short_config = dict(
    ...
    algo_params=dict(
        meta_batch=2, # number of tasks to average the gradient across
        num_iterations=10, # number of data sampling / training iterates
        num_initial_steps=100, # number of transitions collected per task before training
        num_tasks_sample=2, # number of randomly sampled tasks to collect data for each iteration
        ...
        num_steps_per_eval=10,  # nuumber of transitions to eval on
        batch_size=4, # number of transitions in the RL batch
        embedding_batch_size=2, # number of transitions in the context batch
        embedding_mini_batch_size=2, # number of context transitions to backprop through (should equal the arg above except in the recurrent encoder case)
        max_path_length=20, # max path length for this environment
